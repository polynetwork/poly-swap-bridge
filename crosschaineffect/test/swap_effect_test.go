...
func TestSwapEffect_AddSrcTransaction(t *testing.T) {
	...
	dao := crosschaindao.NewCrossChainDao(basedef.SERVER_POLY_SWAP, true, config.DBConfig)
	if dao == nil {
...
func TestSwapEffect_AddDstTransaction(t *testing.T) {
	...
	dao := crosschaindao.NewCrossChainDao(basedef.SERVER_EXPLORER, true, config.DBConfig)
	if dao == nil {
		panic("server is not valid")
	}
